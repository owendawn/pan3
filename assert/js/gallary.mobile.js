...
;(function ( $, window, document, undefined ) {
    ...
    function Plugin( element, options ) {
        ...
        this.bigItemsList = this.galleryContainer.children('ul:eq(0)');
    ...
    Plugin.prototype = {
        ...
        init: function() {
            var that = this,
                smallItems = this.galleryContainer.children('div:eq(0)').find("ul"),
        ...
        bindListHandler: function(smallItems){
            ...
            smallItems.on('click', 'li', function(e){
                ...
                that.current = Number($this.attr("data-idx"));
                that.fadeAllOut();
                that.showControls();
                that.slideshow = true;
                ...
                $this.one('webkitAnimationEnd oanimationend msAnimationEnd animationend', function(e) {
                    if(!$(that.element).find("ul.items--small>li").hasClass("showLastSecond")){
        ...
        fadeAllOut: function(){
            this.galleryContainer.find('ul.items--small')
                     .children('li')
        ...
        fadeAllIn: function(){
            var that = this;
            var dropZone = this.galleryContainer.find('ul.items--small').children('li:eq(' + that.current + ')');
            this.galleryContainer.find('ul.items--small')
                     .children('li')
                     .not(dropZone)
        ...
        quitSlideShow: function(test) {
            ...
            var currentImg = this.galleryContainer.children('ul:eq(0)').children('li:eq(' + that.current + ')'),
                  dropZone = this.galleryContainer.children('div:eq(0)').find('ul>li[data-idx="' + that.current + '"]'),
